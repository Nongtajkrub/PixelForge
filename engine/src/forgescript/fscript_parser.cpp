...
std::optional<ASTNode> Parser::parse_var_declaration_stmt() {
	...
	// Create an identifier for the variable with an unknow type.
	const auto iden_attr =
		this->symbols.new_identifier(
			this->symbols.intern(*this->tokens.advance().lexeme),
			IdenAttr(nullptr, VarAttr()));
	node->identifier = new_identifier_node(iden_attr);

	switch (this->tokens.advance().kind) {
	// Handle standard type annotation.
	case TokenKind::COLON: {
		...
		if (!this->tokens.match_kind(TokenKind::EQUAL)) break;
		[[fallthrough]];
	}
	// Handle automatic type resolving or just parsing initialization expression.
	case TokenKind::EQUAL: {
		const auto expr_loc = this->tokens.peek().location;

		OPT_ASSIGN_OR_RETURN(node->init, pratt_parser());
		const auto expr_type = resolve_expr_type(*node->init, expr_loc);

		if (!expr_type->is_value) {
			emit(DiagnosticKind::EXPECTED_VALUE_TYPE, expr_loc);
			return std::nullopt;
		}
		...
		if (iden_attr->get_data<VarAttr>().type == nullptr) {
			...
			if (iden_attr->get_data<VarAttr>().type != expr_type) {
				emit(DiagnosticKind::TYPE_ERROR, expr_loc);
				return std::nullopt;
			}
		}
	...
	default:
		emit(DiagnosticKind::EXPECTED_TYPE, this->tokens.advance());
		return std::nullopt;
	}
...
std::optional<ASTNode> Parser::parse_func_declaration_stmt() {
	...
	auto attr =
		this->symbols.new_identifier(
			this->symbols.intern(*(this->tokens.advance()).lexeme),
			IdenAttr(nullptr, FuncAttr()));
	node->identifier = new_identifier_node(attr);
...
std::optional<ASTNode> Parser::parse_inter_declaration_stmt() {
	...
	if (this->tokens.match_kind(TokenKind::EXTEND)) {
		...
		const auto extend_type = lookup_type_symbol(this->tokens.advance());
...
std::optional<ASTNode> Parser::parse_loop_stmt() {
	auto node = new_node<LoopStmt>(ASTNodeKind::LOOP);

	// Ignore the "loop" keyword.
	this->tokens.advance();

	if (this->tokens.match_kind(TokenKind::UPDATE)) {
		if (this->symbols.this_scope() != ScopeKind::GLOBAL) {
			emit(DiagnosticKind::LOOP_UPDATE_NOT_ALLOW, this->tokens.prev());
			return std::nullopt;
		}
		node->is_update = true;
	}
	...
	this->symbols.enter_scope(
		(node->is_update) ? ScopeKind::UPDATE : ScopeKind::LOOP);

	OPT_ASSIGN_OR_RETURN(
		node->block,
		parse_block([](auto kind) { return kind == TokenKind::END; }));
...
std::optional<ASTNode> Parser::parse_cmd_stmt(bool as_expr) {
	...
	case CID_COLLIDE:
		this->symbols.new_identifier(
			this->symbols.intern("collider"),
			IdenAttr(this->symbols.types.ty_sprite, VarAttr()));
		break;
	case CID_WAIT:
		if (!this->symbols.in_scope(ScopeKind::UPDATE)) {
			emit(DiagnosticKind::COMMAND_USED_NOT_ALLOW, this->tokens.prev());
			return std::nullopt;
		}
		break;
	default:
	...
	if (!this->tokens.expect(TokenKind::LEFT_PAREN)) return std::nullopt;

	if (!parse_func_call_args(
			node->args, get_command_args(node->id), TokenKind::RIGHT_PAREN))
		return std::nullopt;

	if (!as_expr && !this->tokens.expect(TokenKind::SEMICOLON)) 
		return std::nullopt;
...
std::optional<ASTNode> Parser::pratt_nud() {
	...
	case TokenKind::DOLLAR_SIGN: {
		...
		return new_identifier_node(attr);
	}
	case TokenKind::IDENTIFIER: { 
		const auto attr =
			this->symbols.lookup(
...
bool Parser::parse_func_args(std::vector<ASTNode>& buf, IdenAttr* func_attr) {
	...
	while (this->tokens.ensure_not_eof()) {
		...
		const auto arg_type = parse_type_annotation<FuncArgument>(node);
		...
		if (this->tokens.peek().kind == TokenKind::COMMA) { 
			this->tokens.advance();
		} else if (this->tokens.peek().kind != TokenKind::RIGHT_PAREN) {
...
bool Parser::parse_func_call_args(
	...
	while (!this->tokens.is_eof()) {
		...
		if (this->tokens.peek().kind == TokenKind::COMMA) { 
			this->tokens.advance();
		} else if (this->tokens.peek().kind != terminator) {
			emit(DiagnosticKind::EXPECTED_COMMA, this->tokens.peek());
			return false;
		}
	}
...
bool Parser::ensure_func_returns(const BlockStmt* body) {
	for (const auto& node : body->block) {
		...
		const auto child_blocks = node.get_child_blocks();
		if (child_blocks.empty()) continue;
...
std::vector<TypeAttr*> Parser::get_command_args(command_id_t id) {
	...
	case CID_SPAWN:
	case CID_DESPAWN:
		return {types.ty_sprite};
	case CID_UP:
	case CID_DOWN:
	case CID_RIGHT:
	case CID_LEFT:
		return {types.ty_sprite, types.ty_int};
	case CID_GOTO:
		return {types.ty_sprite, types.ty_int, types.ty_int};
	case CID_COLLIDE:
		return {types.ty_sprite, types.ty_sprite};
	case CID_SHOW:
		return {};
	}
}

TypeAttr* Parser::get_command_returns(command_id_t id) {
	const auto& types = this->symbols.types;
...
TypeAttr* Parser::resolve_expr_type(
	...
	case ASTNodeKind::DOT: {
		...
		const auto object_type = resolve_expr_type(node->object, err_loc);
		if (!object_type) return nullptr;

		const auto prop = object_type->get_prop(node->prop->id);
...
IdenAttr* Parser::lookup_type_symbol(const Token& token) {
	assert(token.kind == TokenKind::IDENTIFIER);

	const auto attr = 
		this->symbols.lookup_global(this->symbols.intern(*token.lexeme));

	if (!attr) {
		emit(DiagnosticKind::UNKNOWN_IDENTIFIER, token.location);
		return nullptr;
	}
...
bool Parser::ensure_expr_type(
	...
	const auto expr_type = resolve_expr_type(expr, err_loc);
